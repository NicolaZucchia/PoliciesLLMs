...
def analyze_policy_category(policy_text, category, api_key):
    """
...
def analyze_policy_with_categories(policy_text, categories, api_key):
    """
    Analyze all categories in the privacy policy.
    """
    output_lines = []
    for category in categories:
        category_output = analyze_policy_category(policy_text, category, api_key)
        if category_output:
            output_lines.append(category_output)
    return "\n".join(output_lines)

def sort_and_process_files(api_key_path, policy_folder, categories_folder, output_folder):
    """
    ...
    try:
        ...
        for policy_file in policy_files:
            ...
            if matching_category:
                ...
                output = analyze_policy_with_categories(policy_text, categories, api_key)
...
if __name__ == "__main__":
    ...
    categories_folder = "../Output/gpt35_turbo/textResults/2shot"  # Folder containing category text files
    output_folder = "gpt35Responses"  # Folder to save GPT responses

    sort_and_process_files(api_key_path, policy_folder, categories_folder, output_folder)
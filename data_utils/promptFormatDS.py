...
PROMPT_TEMPLATE = """
The following text between double quotation marks is a review of a privacy policy:
"{deepseek_txtResult}"

Does the review affirm that any of these personal data are collected?
— Other
— Policy Change
...
Ignore the text between <think> and </think>. If multiple rows mention any of the categories, 
please consider ONLY the last one as definitive, also ignore any other comment that is not the boolean value.
Values can include "true"/"yes"/"TRUE" and other variants for "Yes", and "false"/"no"/"FALSE" and other variants for "No".

Here is an example of processing a privacy policy:

Example Policy:
"{example_deepseek_txtResults}"
